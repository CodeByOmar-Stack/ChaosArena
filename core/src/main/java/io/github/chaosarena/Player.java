...
public class Player {
    ...
    public Player(String name, String atlasPath, float x, float y, boolean facingRight, float worldHeight) {
...
// Queremos que el personaje ocupe aproximadamente el 55% de la altura de la pantalla virtual.

        TextureRegion ref = idleAnim.getKeyFrame(0);

        if (ref != null) {

            float targetHeight = worldHeight * 0.55f;

            this.scale = targetHeight / ref.getRegionHeight();

        } else {

            this.scale = 5.5f; // Valor por defecto si falla la carga
...
// SPECIAL
    ...
    public boolean canHit(Player other) {
        ...
        switch (currentAttackType) {
            ...
            case KICK: reach = 75 * scale; break;

            case SPECIAL: reach = 110 * scale; break;

            case PUNCH:

            default: reach = 55 * scale; break;
        ...
        boolean overlapY = Math.abs(y - other.y) < (110 * scale);
